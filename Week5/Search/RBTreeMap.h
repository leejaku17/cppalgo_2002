...
namespace cppalgo
{
	...
	public:
		...
		// Copy가능하게 하기 위해 Copy Constructor와 operator= 를 추가 
		RBTreeMap(const RBTreeMap& map);
		const RBTreeMap& operator=(const RBTreeMap& map);
	...
	template <class TYPE>
		RBTreeMap<TYPE>::RBTreeMap(const RBTreeMap& map)
	{
		_CopyFrom(map.m_pNodeHead->pLeft);
	}

	template <class TYPE>
		const RBTreeMap<TYPE>& RBTreeMap<TYPE>::operator=(const RBTreeMap& map)
	{
		RemoveAll();
		_CopyFrom(map.m_pNodeHead->pLeft);
		return *this;
	...
	template <class TYPE>
		void RBTreeMap<TYPE>::_CopyFrom(Node* pNode)
	{
		if (pNode != 0)
		{
			Insert(pNode->data);
			_CopyFrom(pNode->pLeft);
			_CopyFrom(pNode->pRight);
		}
	}
};
