...
namespace cppalgo {
	...
	public:
		...
		const SimpleList& operator=(const SimpleList& list);
	...
	template <class TYPE> SimpleList<TYPE>::SimpleList(const SimpleList& list)
	{
	...
	templat <class TYPE>
		const SimpleList<TYPE>& SimpleList<TYPE>::operator=(const SimpleList& list)
	{
		RemoveAll();		
		POS pos = list.GetHeadPosition();
		while (pos) {
