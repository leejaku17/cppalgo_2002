...
namespace cppalgo {
	...
	public:
		...
		const DoubleList& operator=(const DoubleList& list);
	...
	// 추가된 부분 
	template <class TYPE> DoubleList<TYPE>::DoubleList(const DoubleList& list)
	{
		POS pos = list.GetHeadPosition();
		while (pos) {
			TYPE t = list.GetNext(pos);
			AddTail(t);
		}
	}

	template <class TYPE> const DoubleList<TYPE>& DoubleList<TYPE>::operator=(const DoubleList& list)
	{
		RemoveAll();
		POS pos = list.GetHeadPosition();
		while (pos) {
